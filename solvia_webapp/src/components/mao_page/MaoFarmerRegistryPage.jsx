...
export default function MaoFarmerRegistryPage() {
  ...
  const [usersArray, setUsersArray] = useState([]);   // for tables
  const [accounts, setAccounts] = useState({});       // for lookup
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      q,
      (snapshot) => {
        setUsersArray(snapshot.docs.map((d) => ({ id: d.id, ...d.data() })));
        setLoading(false);
      },
      (error) => {
  ...
  /* ==================== FILTER & PAGINATION ==================== */
  const farmerUsers = usersArray.filter((u) => {
    if (u.role !== "Farmer") return false;

    // MAO MUNICIPALITY FILTER
    if (
      currentUserRole === "MAO" &&
      u.Municipality !== currentUserMunicipality
    ) {
      return false;
    }
                    ...
                    {currentRecords.map((u, i) => (
                      <tr key={u.id}>
                        ...
                        <td>
                          ...
                          <img
                            src={
                              u.AvatarIndex !== undefined
                                ? avatars[u.AvatarIndex]
